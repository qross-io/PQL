...
class PQL(val originalSQL: String, val dh: DataHub) {
                ...
                SQL .split(EM$RIGHT)
                    .flatMap(block => {
                        if (block.contains(EM$LEFT)) {
                            ...
                            if ("""^\s*=""".r.test(p2)) {
                                s += "OUTPUT " + p2.takeAfter("=").cleanCommentsAndStashChars(this)
                            }
                            else {
                                s ++= p2.cleanCommentsAndStashChars(this).split(";").map(_.trim)
                            }
    ...
    private def parseOUTPUT(sentence: String): Unit = {
        if ($OUTPUT.test(sentence)) {
            PARSING.head.addStatement(new Statement("OUTPUT", sentence, new OUTPUT(sentence.takeAfter($OUTPUT).trim)))
        }
        else {
    ...
    private def executeOUTPUT(statement: Statement): Unit = {
        RESULT += statement.instance.asInstanceOf[OUTPUT].execute(this)
    }
    ...
    private def executePARSE(statement: Statement): Unit = {
        val table = dh.parseTable(statement.instance.asInstanceOf[PARSE].path.$eval(this).asText)
        RESULT += table
        ROWS = table.size
