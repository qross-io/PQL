...
import cn.qross.net.HttpRequest;
import cn.qross.pql.PQL;
    ...
    public Map<String, Object> request(String path, DataHub dh) {
        ...
        if (attributes != null) {
            ...
            if (OneApi.contains(path, method)) {
                ...
                switch (Setting.OneApiSecurityMode) {
                    case "token":
                        //token
                        String token = OneApi.getToken(request, "token");
                        if (!token.isEmpty()) {
                        ...
                        break;
                    case "secret":
                        String key = OneApi.getToken(request, "secret");
                        //secret key
                        if (!key.isEmpty()) {
