...
import cn.qross.ext.TypeExt;
import cn.qross.fs.*;
import cn.qross.jdbc.DataAccess;
import cn.qross.jdbc.JDBC;
import cn.qross.net.Redis;
import cn.qross.security.Token;
...
public class OneApi {
    ...
    public static String getToken(HttpServletRequest request, String parameter) {
    ...
    public static String getSecretKey(String token) {
        String key;
        if (cn.qross.setting.Properties.contains("redis.qross.host")) {
    ...
    public static boolean authenticateToken(String method, String path, String token) {
        if (Token.TOKENS.containsKey(token)) {
            ...
            if (PERMIT.containsKey("*")) {
                return PERMIT.get("*").contains("*") || PERMIT.get("*").contains(requester);
            }
            else if (PERMIT.containsKey(path)) {
                return PERMIT.get(path).contains("*") || PERMIT.get(path).contains(requester);
            }
            else if (PERMIT.containsKey(request)) {
                return PERMIT.get(request).contains("*") || PERMIT.get(request).contains(requester);
            }
            else {
    ...
    public static String getSetting(String key) {
        return cn.qross.setting.Properties.get(key, null);
    }
