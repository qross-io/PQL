...
import cn.qross.exception.{SharpDataExceptionException, SharpInapplicableLinkNameException, SharpLinkArgumentException}
...
import cn.qross.security.{Base64, MD5}
import cn.qross.setting.Global
import cn.qross.time.ChronExp
...
object Sharp {
    ...
    // DISTINCT a, b
    // DISINCT *
    def DISTINCT(data: DataCell, arg: DataCell, origin: String): DataCell = {
        if (arg.valid) {
            if (arg.isJavaList) {
                data.asTable.distinct(arg.asList[String]: _*).toDataCell(DataType.TABLE)
            }
            else if (arg.isText) {
                data.asTable.distinct(arg.asText).toDataCell(DataType.TABLE)
            }
            else {
                throw SharpLinkArgumentException.occur("DISTINCT", origin)
            }
        }
        else {
            throw SharpLinkArgumentException.occur("DISTINCT", origin)
    ...
    //将TABLE 聚合成一个 LinkedHashMap
    def TO$NESTED$MAP(data: DataCell, arg: DataCell, origin: String): DataCell = {
        if (arg.valid) {
            if (arg.isText) {
                data.asTable.turnToNestedMap(arg.asText).toDataCell(DataType.forClassName("java.util.LinkedHashMap"))
            }
            else {
    ...
    def SET(data: DataCell, arg: DataCell, origin: String): DataCell = {
        if (arg.valid && arg.isRow) {
            ...
            else if (data.is("cn.qross.net.Cookies")) {
                //Cookie.set(arg.asRow)
                null
            }
            else if (data.is("cn.qross.net.Session")) {
...
//        data
    ...
    def MAP(data: DataCell, arg: DataCell, origin: String): DataCell = {
        if (arg.valid) {
            ...
            throw SharpLinkArgumentException.occur(s"MAP", origin)
        }
    }
    ...
    def execute(PQL: PQL): DataCell = {
        ...
        for (i <- links.indices) {
            if (SharpLink.all.contains(links(i).linkName)) {
                ...
                data = classOf[Sharp]
                            .getDeclaredMethod(links(i).linkName, classOf[DataCell], classOf[DataCell], classOf[String]) //classOf[DataCell], classOf[DataCell], classOf[String]  Class.forName("cn.qross.core.DataCell"), Class.forName("cn.qross.core.DataCell"), Class.forName("java.lang.String")
                                .invoke(null,
                                    data,
                                    if (i + 1 < links.length &&
                                            (SharpLink.priorities.contains(links(i).linkName) &&
                ...
                if (argument.contains("=")) {
                    ...
                    DataCell(
                        argument
                            ...
                            .map(item =>
                                //AS 或 函数
                                if ("""(?i)\sAS\s""".r.test(item) || """(?i)^[a-z]+\(.*\)$""".r.test(item)) {
                                    item.popStash(PQL)
                                }
                                else {
