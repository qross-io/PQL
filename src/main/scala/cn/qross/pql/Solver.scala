...
import cn.qross.setting.Language
        ...
        def $compute(PQL: PQL, express: Int = FULL): DataCell = {
            ...
            if (EVALUATIONS.contains(caption)) {
                Class.forName(s"cn.qross.pql.$caption")
                    ...
                    .invoke(Class.forName(s"cn.qross.pql.$caption").getConstructor(classOf[String]).newInstance(sentence), PQL, express.asInstanceOf[java.lang.Integer])
                    .asInstanceOf[DataCell]
            }
