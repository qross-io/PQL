...
import cn.qross.net.{Cookies, Json, Session}
import cn.qross.setting.{Configurations, Environment, Global, Language}
...
object GlobalVariable {
    ...
    def get(name: String, PQL: PQL): DataCell = {
        if (Patterns.GLOBAL_VARIABLES.contains(name)) {
            Class.forName("cn.qross.pql.GlobalVariableDeclaration")
                .getDeclaredMethod(name, Class.forName("cn.qross.pql.PQL"))
                .invoke(null, PQL)
...
object GlobalVariableDeclaration {
    ...
    def MILLISTAMP(PQL: PQL): DataCell = DataCell(DateTime.now.toEpochMilli, DataType.INTEGER)

    def YEAR(PQL: PQL): DataCell = DataCell(DateTime.now.getYear, DataType.INTEGER)
    def MONTH(PQL: PQL): DataCell = DataCell(DateTime.now.getMonth, DataType.INTEGER)
    def DAY(PQL: PQL): DataCell = DataCell(DateTime.now.getDayOfMonth, DataType.INTEGER)

    def HOUR(PQL: PQL): DataCell = DataCell(DateTime.now.getHour, DataType.INTEGER)

    def MINUTE(PQL: PQL): DataCell = DataCell(DateTime.now.getMinute, DataType.INTEGER)

    def SECOND(PQL: PQL): DataCell = DataCell(DateTime.now.getSecond, DataType.INTEGER)
    def MILLISECOND(PQL: PQL): DataCell = DataCell(DateTime.now.getMilli, DataType.INTEGER)
    def MICROSECOND(PQL: PQL): DataCell = DataCell(DateTime.now.getMicro, DataType.INTEGER)
    ...
    def COOKIES(PQL: PQL): DataCell = DataCell(new Cookies(), DataType.forClassName("cn.qross.net.Cookies"))
    def SESSION(PQL: PQL): DataCell = DataCell(new Session(), DataType.forClassName("cn.qross.net.Session"))
    ...
    def THEME(PQL: PQL): DataCell = DataCell(cn.qross.look.Theme.random, DataType.ROW)
