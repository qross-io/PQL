...
//        //Class.forName("cn.qross.pql.GlobalFunctionDeclaration")
//        classOf[GlobalFunctionDeclaration]
//            .getDeclaredMethod(functionName, Class.forName("scala.collection.immutable.List"))
//            .invoke(null, args).asInstanceOf[DataCell]
//    }
...
object GlobalFunctionDeclaration {
    ...
    */
    ...
    def PROPERTIES(args: List[DataCell]): DataCell = {
        if (args.size > 0) {
            DataCell(cn.qross.setting.Properties.get(args.head.asText), DataType.TEXT)
        }
        else {
            throw new SQLParseException(s"Empty or miss arguments at @PROPERTIES, expect 1, actual 0")
        }
    }
}
