...
import cn.qross.exception.{ExtensionNotFoundException, SQLParseException}
import cn.qross.ext.TypeExt._
import cn.qross.fs.SourceFile
import cn.qross.jdbc.{DataSource, JDBC}
import cn.qross.net.Json
import cn.qross.pql.Patterns._
import cn.qross.pql.Solver._
import cn.qross.setting.{Configurations, Global, Language}
...
class PQL(val originalSQL: String, val embedded: Boolean, val dh: DataHub) {
    ...
    def parseStatement(sentence: String): Unit = {
        ...
        if (NON_QUERY_CAPTIONS.contains(caption)) {
            ...
            try {
                Class.forName(s"cn.qross.pql.$caption")
                        .getDeclaredMethod("parse", classOf[String], classOf[PQL]) //Class.forName("cn.qross.pql.PQL")
        ...
        breakable {
            for (statement <- statements) {
                if (NON_QUERY_CAPTIONS.contains(statement.caption)) {
                    ...
                    Class.forName(s"cn.qross.pql.${statement.caption}")
