...
import cn.qross.exception.{IncorrectDataSourceException, SQLParseException, UnsupportedSentenceException}
...
import cn.qross.pql.{Patterns, Syntax}
    ...
    def useTemplate(fileNameOrFullPathOrTemplateName: String): Excel = {
        ...
        if (!templatePath.endsWith(".xlsx") && !templatePath.endsWith(".xls")) {
            if (JDBC.hasQrossSystem) {
                ...
                if (ds.tableExists("qross_excel_templates")) {
